...
public class cube implements BlueSentryListener{
	...
	public void sendEvent( int side ){
		try{
			if( side != -1 ){
				if( side == last_side ){  // this is a repeat, increase the count towards sleeping
					count++;
				...
				else{ // this is a new side
					last_side = side; // debounce this side to minimize errors
					count = 0;
				}
			}
			else{ // not a side, restart the count
				count = 0;
			}
	...
	public void readCalibrationData(){
		// read the side data from the file
		try{
