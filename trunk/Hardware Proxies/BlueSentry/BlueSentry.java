...
public class BlueSentry implements Runnable, SerialPortEventListener, CommPortOwnershipListener {
    ...
    //private final String INIT = TOGGLE_AD + NO_PROMPT + SPEED_UP + SPEED_UP + SPEED_UP + SPEED_UP + SLEEP;
    private final String INIT = TOGGLE_AD + SPEED_UP + SPEED_UP + SPEED_UP;
    // numeric chars set up the number of channels ( 8 total );
    ...
    public static void main(String[] args) {
        ...
        // route keyboard input to the bluesentry
    ...
    public void serialEvent(SerialPortEvent event) {
        ...
        case SerialPortEvent.DATA_AVAILABLE:
            try {			
                ...
                // the first token should be a read counter, not a channel reading
                numChannels = numTokens - 1;
                
                // check to make sure the first token is a read counter
                if( st.hasMoreTokens() ){
                    ...
                    try{
                        ...
                        if( DEBUG )
                            System.out.println(getChannel(0));
                    } catch ( NumberFormatException ex ){
                        // ignore "Nite Nite" and "Crank 'er back up" messages
                    }
                }
                reconnectTimer = new Timer();
                reconnectTimer.schedule( new ReconnectTask(), RECONNECT_TIMEOUT );
    ...
    public void register(BlueSentryListener lt) {
		if(clients.contains(lt) == false)
			clients.add(lt);
    }
	
	public void deregister(BlueSentryListener lt) {
        if(clients.contains(lt) == true)
			clients.remove(lt);
    }
