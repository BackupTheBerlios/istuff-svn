...
public class MPProxy implements EventCallback
{
		...
		public MPProxy(String ip, String proxyID, String cmprt)
		{
			try
			{
		...
		{
			MPProxy mobileProxy = null;
			Shutdown killer;
            //Stdio keys;

			if(argv.length == 2) // Then EventHeapName and COM port were supplied
				mobileProxy = new MPProxy(argv[0],"",argv[1]);
				else if (argv.length >= 3) // Also a ProxyID was provided 
					mobileProxy = new MPProxy(argv[0], argv[1], argv[2]);
			
			if (mobileProxy != null){ // The parameters were valid
			...
			{
			System.out.println("Usage: java MPProxy <Event Heap IP> [ProxyID] <COM Port> \n" +
