...
public class ProxyTree
{
	...
	public void insertNode (ProxyNode node){
        ...
        if (predecessorNode != null)
        	...
        	//This is done in the add-method
	...
	private ProxyNode searchPredecessorNode (ProxyNode startNode, ProxyNode searchNode) {
		for ( Iterator<ProxyNode> i = startNode.getSuccessors().iterator() ; i.hasNext(); )
		{
			...
			{
				// If the kind already exists, check wether the proxy class already exists 
					for ( ListIterator<ProxyNode> it = compareKindNode.getSuccessors().listIterator() ; it.hasNext(); )
					{
						...
						if (compareProxyClassNode.getproxyClass().equals(searchNode.getproxyClass()))
						{
							...
							for ( Iterator<ProxyNode> proxyIterator = compareProxyClassNode.getSuccessors().iterator() ; proxyIterator.hasNext(); )
							{
								ProxyNode compareProxy = proxyIterator.next();
								if (compareProxy.getPathToFile().equals(searchNode.getPathToFile()))
								{
//									 This means that the same node is already in the tree
									return null;
								}
								
							}
							return compareProxyClassNode;
							//  if the node does not exist in the tree, return the ProxyClassNode
	...
	public boolean isInTree(ProxyNode searchNode){
		for ( Iterator<ProxyNode> kindIterator = getTreeRoot().getSuccessors().iterator() ; kindIterator.hasNext(); )
		{
			  ...
			  return (true);
				...
				// node is a proxy class
					for ( Iterator<ProxyNode> pClassIterator = compareKindNode.getSuccessors().iterator() ; pClassIterator.hasNext(); )
					{
						...
						{
							ProxyNode compareProxyNode = (ProxyNode) proxyIterator.next();
