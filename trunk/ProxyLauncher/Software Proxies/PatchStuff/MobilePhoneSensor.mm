...
#import "MobilePhoneSensor.h"

@interface MobilePhoneSensor (QCInspector)
+ (Class)inspectorClassWithIdentifier:(id)fp8;
@end

@implementation MobilePhoneSensor

+ (Class)inspectorClassWithIdentifier:(id)fp8
{
	return [MobilePhoneSensorUI class];
}
...
- (id)initWithIdentifier:(id)fp8
{
	...
	NSLog (@"Initializing MobilePhoneSensor");
	...
	eh2_init ("iStuffQuartzPlugin", "localhhost");
	...
	NSLog(@"About to create EH for MobilePhoneSensor");
	NSString *serverName = @"localhost";
	[self createEventHeap:NULL atServer:serverName atPort:4535];
	
	NSLog (@"created EH for MobilePhoneSensor  -  trying to generate new output port");
	QCNumberPort *outputtestport;
		outputtestport = [QCNumberPort QCOpenGLContext];
		[outputtestport setDoubleValue:0.0];
...
- (void) waitForEvents
{
	...
	while (waitForEvents) {
		...
		char* taggie = (char*) resultEventPtr->getPostValueString("ParticleSrcId");
