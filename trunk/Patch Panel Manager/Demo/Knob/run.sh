...
then
	...
	echo Beware that MACHINE_NAME should correspond to the global variable "currentHostName" of the KnobScript.pps- patch panel script
	echo
	exit 1
fi
EHEAPNAME=$1
...
echo
echo Start Knob Proxy
#....to be cont
echo
echo Load patch script
java -cp $LIBPATH/iROS.jar:$LIBPATH/jph.jar iwork.patchpanel.manager.PPManager -s KnobScript.pps $EHEAPNAME &
